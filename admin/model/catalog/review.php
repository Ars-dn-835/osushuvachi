...
class ModelCatalogReview extends Model {
	...
	public function getReviews($data = array()) {
		$sql = "SELECT r.review_id, pd.name, r.author, r.email, r.advantages, r.disadvantages, r.rating, r.status, r.date_added FROM " . DB_PREFIX . "review r LEFT JOIN " . DB_PREFIX . "product_description pd ON (r.product_id = pd.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		...
		if (!empty($data['filter_advantages'])) {
			$sql .= " AND r.advantages LIKE '" . $this->db->escape($data['filter_advantages']) . "%'";
		}

		if (!empty($data['filter_disadvantages'])) {
			$sql .= " AND r.disadvantages LIKE '" . $this->db->escape($data['filter_disadvantages']) . "%'";
		}
