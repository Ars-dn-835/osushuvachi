...
class ControllerCatalogReview extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['filter_advantages'])) {
				$url .= '&filter_advantages=' . urlencode(html_entity_decode($this->request->get['filter_advantages'], ENT_QUOTES, 'UTF-8'));
			}

			if (isset($this->request->get['filter_disadvantages'])) {
				$url .= '&filter_disadvantages=' . urlencode(html_entity_decode($this->request->get['filter_disadvantages'], ENT_QUOTES, 'UTF-8'));
			}
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_advantages'])) {
			$filter_advantages = $this->request->get['filter_advantages'];
		} else {
			$filter_advantages = '';
		}

		if (isset($this->request->get['filter_disadvantages'])) {
			$filter_disadvantages = $this->request->get['filter_disadvantages'];
		} else {
			$filter_disadvantages = '';
		}
		...
		$filter_data = array(
			...
			'filter_advantages'     => $filter_advantages,
			'filter_disadvantages'     => $filter_disadvantages,
		...
		$data['sort_email'] = $this->url->link('catalog/review', 'user_token=' . $this->session->data['user_token'] . '&sort=r.email' . $url, true);
		$data['sort_advantages'] = $this->url->link('catalog/review', 'user_token=' . $this->session->data['user_token'] . '&sort=r.advantages' . $url, true);
		$data['sort_disadvantages'] = $this->url->link('catalog/review', 'user_token=' . $this->session->data['user_token'] . '&sort=r.disadvantages' . $url, true);
		...
		$data['filter_advantages'] = $filter_advantages;
		$data['filter_disadvantages'] = $filter_disadvantages;
	...
	protected function getForm() {
		...
		if (isset($this->error['advantages'])) {
			$data['error_advantages'] = $this->error['advantages'];
		} else {
			$data['error_advantages'] = '';
		}

		if (isset($this->error['disadvantages'])) {
			$data['error_disadvantages'] = $this->error['disadvantages'];
		} else {
			$data['error_disadvantages'] = '';
		}
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['email']) < 0) || (utf8_strlen($this->request->post['email']) > 100)) {
			$this->error['email'] = $this->language->get('error_email');
		}

		if ((utf8_strlen($this->request->post['advantages']) < 0) || (utf8_strlen($this->request->post['advantages']) > 210)) {
			$this->error['advantages'] = $this->language->get('error_advantages');
		}

		if ((utf8_strlen($this->request->post['disadvantages']) < 0) || (utf8_strlen($this->request->post['disadvantages']) > 100)) {
			$this->error['disadvantages'] = $this->language->get('error_disadvantages');
		}
