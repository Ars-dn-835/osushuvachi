				...
				switch ($preset) {
                    ...
                    case 'buywith':
                        switch (Arr::get($this->request->get, 'route')) {
                            case 'journal3/blog/post':
                                $post_id = (int)Arr::get($this->request->get, 'journal_blog_post_id');
                                $results = $this->model_journal3_blog->getBuyWithProducts($post_id, $limit);
                                break;

                            case 'product/product':
                                $product_id = (int)Arr::get($this->request->get, 'product_id');
                                $results = $this->model_journal3_product->getBuyWithProducts($product_id, $limit);
                                break;

                            case 'checkout/cart':
                            case 'checkout/checkout':
                                $product_ids = $this->cart->getProducts();
                                $results = $this->model_journal3_product->getBuyWithProducts($product_ids, $limit);
                                break;
	...
	private function parseProducts($results) {
		...
		foreach ($results as $result) {
            ...
            if (!empty($data['attribute_groups']) && isset($data['attribute_groups'][0]['attribute'])) {
                ...
                $productarray = $data['attribute_groups'][0]['attribute'];


//                    $originalArray = $data['attribute_groups'][0]['attribute'];
//                    $halfCount = ceil(count($originalArray) / 2);
//
//                    for ($i = 0; $i < $halfCount; $i++){
//                        $productarray[$i]['attribute_id'] = $data['attribute_groups'][0]['attribute'][$i * 2]['attribute_id'];
//                        $productarray[$i]['first_name'] = $data['attribute_groups'][0]['attribute'][$i * 2]['name'];
//                        $productarray[$i]['first_text'] = $data['attribute_groups'][0]['attribute'][$i * 2]['text'];
//                        if (isset($data['attribute_groups'][0]['attribute'][$i * 2 + 1]['name'])) {
//                            $productarray[$i]['attribute_id'] = $data['attribute_groups'][0]['attribute'][$i * 2 + 1]['attribute_id'];
//                            $productarray[$i]['second_name'] = $data['attribute_groups'][0]['attribute'][$i * 2 + 1]['name'];
//                            $productarray[$i]['second_text'] = $data['attribute_groups'][0]['attribute'][$i * 2 + 1]['text'];
//                        } else {
//                            $productarray[$i]['attribute_id'] = '';
//                            $productarray[$i]['second_name'] = '';
//                            $productarray[$i]['second_text'] = '';
//                        }
//                    }
//                    $data['attribute_one'] = $productarray;
                }

//$this->log->write('$productarray  ' . print_r($productarray,1));
