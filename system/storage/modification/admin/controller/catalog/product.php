...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
                    ...
                    $this->load->language('extension/module/youtube_product_video_edit');
    ...
    // SEO URL Generator PRO . begin
		...
		$data['languages'] = $this->model_localisation_language->getLanguages();
                    ...
                    if (isset($this->request->post['youtube_product_video'])) {
			            $data['youtube_product_video'] = html_entity_decode($this->request->post['youtube_product_video'], ENT_QUOTES, 'UTF-8');
		            } elseif (isset($product_info['youtube_product_video'])) {
			            $data['youtube_product_video'] = html_entity_decode($product_info['youtube_product_video'], ENT_QUOTES, 'UTF-8');
		            } else {
			            $data['youtube_product_video'] = '';
		            }
                    // YouTube Product Video -- End
	   ...
	   // payment_pb_chastyami
		if (isset($this->request->post['payment_pb_chastyami'])) {
			$data['payment_pb_chastyami'] = $this->request->post['payment_pb_chastyami'];
		} elseif (!empty($product_info)) {
			$data['payment_pb_chastyami'] = $product_info['payment_pb_chastyami'];
		} else {
			$data['payment_pb_chastyami'] = 1;
		}
	   // payment_pb_rassrochka
		if (isset($this->request->post['payment_pb_rassrochka'])) {
			$data['payment_pb_rassrochka'] = $this->request->post['payment_pb_rassrochka'];
		} elseif (!empty($product_info)) {
			$data['payment_pb_rassrochka'] = $product_info['payment_pb_rassrochka'];
		} else {
			$data['payment_pb_rassrochka'] = 1;
		}
	   // payment_mono_chastyami
		if (isset($this->request->post['payment_mono_chastyami'])) {
			$data['payment_mono_chastyami'] = $this->request->post['payment_mono_chastyami'];
		} elseif (!empty($product_info)) {
			$data['payment_mono_chastyami'] = $product_info['payment_mono_chastyami'];
		} else {
			$data['payment_mono_chastyami'] = 1;
		}
        ...
        $this->log->write('$this->request->post' . print_r($this->request->post, 1));
		...
		foreach ($product_attributes as $product_attribute) {
            ...
            $keys = array_keys($product_attribute['product_attribute_description']);
            $firstKey = $keys[0];
			if ($attribute_info) {
				$data['product_attributes'][] = array(
                    ...
                    'top' => $product_attribute['product_attribute_description'][$firstKey]['top']
				);
			}
		}
		...
		if (isset($this->request->post['product_buywith'])) {
			$products = $this->request->post['product_buywith'];
		} elseif (isset($this->request->get['product_id'])) {
			$products = $this->model_catalog_product->getProductBuywith($this->request->get['product_id']);
		} else {
			$products = array();
		}

		$data['product_buywiths'] = array();

		foreach ($products as $product_id) {
			$buywith_info = $this->model_catalog_product->getProduct($product_id);

			if ($buywith_info) {
				$data['product_buywiths'][] = array(
					'product_id' => $buywith_info['product_id'],
					'name'       => $buywith_info['name']
				);
			}
		}
		...
		$data['module_seo_url_generator_language'] = $this->config->get('module_seo_url_generator_language');
