...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', payment_pb_chastyami = '" . (int)$data['payment_pb_chastyami'] . "', payment_pb_rassrochka = '" . (int)$data['payment_pb_rassrochka'] . "', payment_mono_chastyami = '" . (int)$data['payment_mono_chastyami'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW(), date_modified = NOW()");
		
		$product_id = $this->db->getLastId();

                    // YouTube Product Video -- Start
                    if (isset($data['youtube_product_video'])) {
                        $this->db->query("INSERT INTO `" . DB_PREFIX . "youtube_product_video` SET `product_id`='" . (int)$product_id . "', `product_video`='" . $this->db->escape($data['youtube_product_video']) . "'");
                    }
		...
		if (isset($data['product_attribute'])) {
			foreach ($data['product_attribute'] as $product_attribute) {
				if ($product_attribute['attribute_id']) {
                    ...
                    if (isset($product_attribute['attribute_top']) && $product_attribute['attribute_top']) {
                        $product_attribute['attribute_top'] = 1;
                    } else {
                        $product_attribute['attribute_top'] = 0;
                    }

					foreach ($product_attribute['product_attribute_description'] as $language_id => $product_attribute_description) {
						$this->db->query("DELETE FROM " . DB_PREFIX . "product_attribute WHERE product_id = '" . (int)$product_id . "' AND attribute_id = '" . (int)$product_attribute['attribute_id'] . "' AND language_id = '" . (int)$language_id . "'");

						$this->db->query("INSERT INTO " . DB_PREFIX . "product_attribute SET product_id = '" . (int)$product_id . "', attribute_id = '" . (int)$product_attribute['attribute_id'] . "', language_id = '" . (int)$language_id . "', text = '" .  $this->db->escape($product_attribute_description['text']). "', top = '" .  (int)$product_attribute['attribute_top'] . "'");
					}
				}
			}
		}
        ...
        if (isset($data['product_buywith'])) {
            foreach ($data['product_buywith'] as $buywith_id) {
                $this->db->query("DELETE FROM " . DB_PREFIX . "product_buywith WHERE product_id = '" . (int)$product_id . "' AND buywith_id = '" . (int)$buywith_id . "'");
                $this->db->query("INSERT INTO " . DB_PREFIX . "product_buywith SET product_id = '" . (int)$product_id . "', buywith_id = '" . (int)$buywith_id . "'");
                $this->db->query("DELETE FROM " . DB_PREFIX . "product_buywith WHERE product_id = '" . (int)$buywith_id . "' AND buywith_id = '" . (int)$product_id . "'");
                $this->db->query("INSERT INTO " . DB_PREFIX . "product_buywith SET product_id = '" . (int)$buywith_id . "', buywith_id = '" . (int)$product_id . "'");
            }
        }
		...
		foreach ($data['product_seo_url'] as $store_id => $store) {
			foreach ($store as $language_id => $url) {
				if (empty($data['product_seo_url'][$store_id][$language_id])) {
					$sug_data = array(
						'name'            => $data['product_description'][$data['store_setting'][$store_id]['language'][$language_id]]['name'],
						'model'           => $data['model'],
...
//		}
	...
	public function editProduct($product_id, $data) {
                    ...
                    // YouTube Product Video -- Start
                    if (isset($data['youtube_product_video'])) {
                        $this->db->query("DELETE FROM `" . DB_PREFIX . "youtube_product_video` WHERE `product_id` = '" . (int)$product_id . "'");
                        $this->db->query("INSERT INTO `" . DB_PREFIX . "youtube_product_video` SET `product_id`='" . (int)$product_id . "', `product_video`='" . $this->db->escape($data['youtube_product_video']) . "'");
                    }
                    // YouTube Product Video -- End
		...
		if (!empty($data['product_attribute'])) {
			foreach ($data['product_attribute'] as $product_attribute) {
				if ($product_attribute['attribute_id']) {
					...
					foreach ($product_attribute['product_attribute_description'] as $language_id => $product_attribute_description) {
						$this->db->query("INSERT INTO " . DB_PREFIX . "product_attribute SET product_id = '" . (int)$product_id . "', attribute_id = '" . (int)$product_attribute['attribute_id'] . "', language_id = '" . (int)$language_id . "', text = '" .  $this->db->escape($product_attribute_description['text']). "', top = '" .  $product_attribute['attribute_top'] . "'");
					}
				}
			}
		}
        ...
        $this->db->query("DELETE FROM " . DB_PREFIX . "product_buywith WHERE product_id = '" . (int)$product_id . "'");
        $this->db->query("DELETE FROM " . DB_PREFIX . "product_buywith WHERE buywith_id = '" . (int)$product_id . "'");
...
//		}
	...
	public function copyProduct($product_id) {
		...
		if ($query->num_rows) {
			...
			$data['status'] = '0';
                    ...
                    // YouTube Product Video -- Start
                    $youtube_product_video_row = $this->db->query("SELECT `product_video` FROM `" . DB_PREFIX . "youtube_product_video` WHERE `product_id` = '" . (int)$product_id . "'")->row;
                    $data['youtube_product_video'] = ( isset($youtube_product_video_row['product_video']) ) ? $youtube_product_video_row['product_video'] : null;
                    // YouTube Product Video -- End
			...
			$data['product_buywith'] = $this->getProductBuywith($product_id);
	...
	public function getProduct($product_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
                    ...
                    // YouTube Product Video -- Start
                    $youtube_product_video_row = $this->db->query("SELECT `product_video` FROM `" . DB_PREFIX . "youtube_product_video` WHERE `product_id` = '" . (int)$product_id . "'")->row;
                    $query->row['youtube_product_video'] = ( isset($youtube_product_video_row['product_video']) ) ? $youtube_product_video_row['product_video'] : '';
                    // YouTube Product Video -- End
		...
		foreach ($product_attribute_query->rows as $product_attribute) {
            ...
            foreach ($product_attribute_description_query->rows as $product_attribute_description) {
                ...
                $product_attribute_description['language_id']
                    ...
                    'top' => $product_attribute_description['top'],
                );
	...
	public function getProductBuywith($product_id) {
		$product_buywith_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_buywith WHERE product_id = '" . (int)$product_id . "'");

		foreach ($query->rows as $result) {
			$product_buywith_data[] = $result['buywith_id'];
		}

		return $product_buywith_data;
	}
