...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
                    ...
                    $data['youtube_product_video'] = $this->load->controller('extension/module/youtube_product_video', $product_id);
        ...
        if(!isset($category_info)) {
          ...
          if($categories) {
			...
			$data['text__reviews'] = sprintf($this->language->get('text__reviews'), $product_info['reviews']);
			...
			foreach ($results as $result) {
$this->log->write('images 288  ' . print_r($result, 1));
				$data['images'][] = array(
					'popup' => $this->model_tool_image->resize($result['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_popup_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_popup_height')),
					'thumb' => $this->model_tool_image->resize($result['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_additional_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_additional_height')),
                    'video' => $this->model_tool_image->resize($result['video'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_additional_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_additional_height'))
            ...
            // fjeogjsjgksejgkgesjk
            $data['difference'] = !is_null($product_info['special']) ? $product_info['price'] - $product_info['special'] : null;
            // 4drhdrhdrhrdhdrhhrdhrd
            ...
            $data['disadvantages'] = array();

			$data['reviews'] = sprintf($this->language->get('text_reviews'), (int)$product_info['reviews']);
			
//			$data['email'] = sprintf($this->language->get('text_email'), (int)$product_info['email']);
//			$data['advantages'] = sprintf($this->language->get('text_advantages'), (int)$product_info['advantages']);
//			$data['disadvantages'] = sprintf($this->language->get('text_disadvantages'), (int)$product_info['disadvantages']);
			...
			$data['attribute_groups'] = $this->model_catalog_product->getProductAttributesTop($this->request->get['product_id']);

            $data['attribute_one'] = array();

            if (!empty($data['attribute_groups']) && isset($data['attribute_groups'][0]['attribute'])) {

                $data['attribute_one'] = $data['attribute_groups'][0]['attribute'];
