...
class __TwigTemplate_0e87886846a509aa0b137d5e5d376c410c0d06c033d6175a297eae6ce001752f extends \Twig\Template
{
    private $source;
    private $macros = [];
...
<script>
    \$(document).ready(function() {
        \$('#simplecheckout_button_confirm').on('click', function() {
            // Отримайте дані, які вам потрібно передати методу validate.
            var shippingMethod = \$('#shipping_method').val(); // Приклад, отримання значення поля з доставкою.

            // Створіть об'єкт з даними, які ви хочете передати.
            var requestData = {
                shipping_method: shippingMethod
                // Додайте інші дані, які вам потрібно передати.
            };

            // Відправте AJAX-запит на метод validate.
            \$.ajax({
                url: 'index.php?route=checkout/simplecheckout_shipping/validate',
                type: 'post',
                data: requestData,
                dataType: 'json',
                success: function(json) {
                    // Обробіть результати, отримані від методу validate.
                    if (json.error) {
                        // Якщо є помилки, обробіть їх тут.
                        alert('Помилка: ' + json.error);
                    } else {
                        // Якщо все в порядку, продовжіть з оформленням замовлення.
                        \$('#simplecheckout_form').submit(); // Приклад: відправка форми оформлення замовлення.
                    }
                },
                error: function(xhr, ajaxOptions, thrownError) {
                    // Обробіть помилки відправлення запиту тут.
                    alert('Помилка відправлення запиту: ' + thrownError);
                }
            });
        });
    });
